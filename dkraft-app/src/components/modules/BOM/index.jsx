...
import { Icon, SearchBox, Modal, InfiniteScrollSentinel } from "../../common";
...
const BOMModule = () => {
  ...
  const getStockStatusStyle = (status) => {
    switch (status) {
      case "sufficient":
        return { color: "#10b981", icon: "check_circle", label: "Sufficient" };
      case "low":
        return { color: "#f59e0b", icon: "warning", label: "Low Stock" };
      case "insufficient":
        return { color: "#ef4444", icon: "error", label: "Insufficient" };
      default:
        return { color: "#64748b", icon: "help", label: "N/A" };
    }
  };
  ...
  const {
    visibleItems: visibleBOMs,
    visibleCount: visibleBOMsCount,
    hasMore: hasMoreBOMs,
    sentinelRef: bomsSentinelRef,
  } = useInfiniteScroll(sortedBOMs, {
    initialCount: 12,
    step: 12,
    resetKey: `${searchTerm}|${sortConfig.key}|${sortConfig.direction}|${viewMode}`,
  });
  ...
  const handleSelectAll = (e) => {
    if (e.target.checked) {
      setSelectedBOMs(visibleBOMs.map((b) => b.id));
    } else {
      setSelectedBOMs([]);
    }
  };
  ...
  const handleSave = async () => {
    try {
      ...
      if (modalMode === "add") {
        ...
        setBoms((prev) => [
          ...prev,
          { ...currentBOM, ...costs, id: newBOM.id },
        ]);
      } else if (modalMode === "edit") {
        await bomService.update(currentBOM.id, bomToSave);
        setBoms((prev) =>
          prev.map((b) =>
            b.id === currentBOM.id ? { ...currentBOM, ...costs } : b,
          ),
        );
      }
      setShowModal(false);
  ...
  const handleRemoveComponent = (index) => {
    const updatedComponents = currentBOM.components.filter(
      (_, i) => i !== index,
    );
    const costs = calculateCosts(
      updatedComponents,
  ...
  const totalMaterialCost = boms.reduce(
    (sum, b) => sum + (b.totalMaterialCost || 0),
    0,
  );
  const lowStockBOMs = boms.filter((b) =>
    b.components?.some(
      (c) => c.stockStatus === "low" || c.stockStatus === "insufficient",
    ),
  ).length;
    ...
    <div className="module-page bom-page">
      ...
      {isLoading ? (
        ...
        <div className="materials-cards-grid">
          {visibleBOMs.map((bom) => {
            const hasLowStock = bom.components?.some(
              (c) =>
                c.stockStatus === "low" || c.stockStatus === "insufficient",
            );
            return (
              <div key={bom.id} className="material-card">
                <div className="material-card-header">
                  ...
                  <div className="material-card-badges">
                    <span
                      className={`status-badge ${getStatusClass(bom.status)}`}
                    >
                ...
                <div className="material-card-footer">
                  <div className="material-stock">
                    ...
                    <span className="stock-value">
                      ${bom.suggestedPrice?.toLocaleString()}
                    </span>
                  </div>
        ...
        <div className="materials-table bom-table">
          <div className="materials-table-header">
            <span className="col-checkbox">
              <input
                ...
                checked={
                  visibleBOMs.length > 0 &&
                  selectedBOMs.length === visibleBOMs.length
                }
            ...
            <span
              className="col-product sortable"
          ...
          {visibleBOMs.map((bom) => {
            ...
            const stockStatus = hasLowStock
              ? getStockStatusStyle("low")
              : getStockStatusStyle("sufficient");

            return (
              <div key={bom.id} className="bom-row-container">
                <div
                  className={`materials-table-row ${isExpanded ? "expanded" : ""}`}
                >
                  <span className="col-checkbox">
                    <input
                      type="checkbox"
                  ...
                  <span
                    className={`col-status status-badge ${getStatusClass(bom.status)}`}
                  >
                    <span className="status-dot"></span>
                    {bom.status}
                  </span>
                  <span className="col-stock-status" title={stockStatus.label}>
                    <Icon
                      ...
                      style={{ color: stockStatus.color, fontSize: "20px" }}
                    />
                  </span>
                ...
                {isExpanded && bom.components && bom.components.length > 0 && (
                  <div className="bom-components-expanded">
                    ...
                    {bom.components.map((comp, idx) => {
                      const compStockStatus = getStockStatusStyle(
                        comp.stockStatus,
                      );
                      return (
                        <div key={comp.id || idx} className="component-row">
                          ...
                          <span
                            className="comp-stock"
                            title={compStockStatus.label}
                          >
                            <Icon
                              name={compStockStatus.icon}
                              style={{ color: compStockStatus.color }}
                            />
                            {comp.stockStatus !== "n/a" && (
                              <span>{comp.stockAvailable}</span>
                            )}
                          </span>
                        </div>
                      );
                    ...
                    <div className="components-footer">
                      ...
                      <span className="footer-value">
                        ${bom.totalMaterialCost?.toLocaleString()}
                      </span>
                      <span className="footer-label">Labor:</span>
                      <span className="footer-value">
                        ${bom.laborCost?.toLocaleString()}
                      </span>
                      <span className="footer-label">Overhead:</span>
                      <span className="footer-value">
                        ${bom.overheadCost?.toLocaleString()}
                      </span>
                      <span className="footer-label total">Total:</span>
                      <span className="footer-value total">
                        ${bom.totalCost?.toLocaleString()}
                      </span>
                    </div>
                  </div>
      ...
      <InfiniteScrollSentinel
        sentinelRef={bomsSentinelRef}
        hasMore={hasMoreBOMs}
        total={sortedBOMs.length}
        visibleCount={visibleBOMsCount}
        label="Loading more BOMs..."
      />
                    ...
                    <div key={comp.id || index} className="component-item">
                      <div className="component-info">
                        ...
                        <div className="component-details">
                          ...
                          <span className="component-meta">
                            {comp.itemCode} | {comp.quantity} {comp.unit} @ $
                            {comp.unitCost?.toLocaleString()} = $
                            {comp.totalCost?.toLocaleString()}
                          </span>
                        </div>
                      ...
                      {modalMode !== "view" && (
                        <div className="component-actions">
                          <button
                            className="btn-icon"
                            onClick={() => handleEditComponent(comp, index)}
                          >
                            <Icon name="edit" />
                          </button>
                          <button
            ...
            <div className="costs-grid">
              <div className="cost-item">
                ...
                <span className="cost-value">
                  $
                  {currentBOM.totalMaterialCost?.toLocaleString(undefined, {
                    minimumFractionDigits: 2,
                  })}
                </span>
              </div>
              ...
              <div className="cost-item">
                ...
                <span className="cost-value">
                  $
                  {currentBOM.overheadCost?.toLocaleString(undefined, {
                    minimumFractionDigits: 2,
                  })}
                </span>
              </div>
              <div className="cost-item total">
                ...
                <span className="cost-value">
                  $
                  {currentBOM.totalCost?.toLocaleString(undefined, {
                    minimumFractionDigits: 2,
                  })}
                </span>
              </div>
              <div className="cost-item editable">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleInputChange("margin", parseFloat(e.target.value) || 0)
                  }
                  min="0"
                  max="100"
                  disabled={modalMode === "view"}
              ...
              <div className="cost-item suggested">
                ...
                <span className="cost-value">
                  $
                  {currentBOM.suggestedPrice?.toLocaleString(undefined, {
                    minimumFractionDigits: 2,
                  })}
                </span>
              </div>
        ...
        <div className="component-form">
          ...
          {currentComponent.type === "material" ? (
            <div className="form-group">
              ...
              <select
                ...
                onChange={(e) =>
                  handleComponentChange("itemId", e.target.value)
                }
              >
            ...
            <div className="form-group">
              ...
              <select
                ...
                onChange={(e) =>
                  handleComponentChange("subBomId", e.target.value)
                }
              >
                <option value="">Choose a BOM</option>
                {boms
                  .filter(
                    (b) => b.id !== currentBOM.id && b.status === "Active",
                  )
                  .map((bom) => (
          ...
          {currentComponent.type === "material" && (
            <div className="stock-info">
              <Icon
                name={getStockStatusStyle(currentComponent.stockStatus).icon}
                style={{
                  color: getStockStatusStyle(currentComponent.stockStatus)
                    .color,
                }}
              />
              <span>
                Stock Available: {currentComponent.stockAvailable || 0}
              </span>
      ...
      >
        <div className="delete-confirm">
          <p>
            Are you sure you want to delete <strong>{bomToDelete?.name}</strong>
            ?
          </p>
