...
/**
 * Login to backend API using NextAuth credentials flow
 * @param {string} email
 ...
 * @returns {Promise<{user: object}>}
 */
export const registerToBackend = async (name, email, password, role = 'USER') => {
    console.log('[API] registerToBackend called with:', { name, email, role });
    ...
    console.log('[API] Register response content-type:', contentType);

    if (!contentType || !contentType.includes('application/json')) {
        const text = await response.text();
        console.error('[API] Register returned non-JSON response:', text.substring(0, 500));
        throw new Error('El endpoint de registro no está disponible en el servidor');
    }
    ...
    try {
        ...
        console.error('[API] Failed to parse register response:', e);
...
const request = async (endpoint, options = {}) => {
    ...
    // 1. { success: true, data: [...] } - return data
    // 2. { data: [...] } - return data
    // 3. [...] - return the array directly
    // 4. { success: false, error: "..." } - throw error
    ...
    // Return result directly if it's an array or doesn't have a data property
    return result;
};
