...
import { Icon, InfiniteScrollSentinel } from "../../common";
...
const QualityModule = () => {
  ...
  const availableYears = useMemo(() => {
    const years = new Set(
      inspections.map((i) => new Date(i.inspectionDate).getFullYear()),
    );
  ...
  const hasPendingItems = (inspection) => {
    const isPendingInspection =
      inspection.result === "Pending" || inspection.status === "Pending";
    const hasOpenFindings = inspection.findings?.some(
  ...
  const isArchivable = (inspection) => {
    ...
    const isCompleted =
      inspection.result !== "Pending" && !hasPendingItems(inspection);
  ...
  const metrics = useMemo(() => {
    ...
    const passedWithObs = inspections.filter(
      (i) => i.result === "Passed with Observations",
    ).length;
    ...
    const reworkRequired = allFindings.filter(
      (f) => f.type === "Rework Required",
    ).length;
    const scrapCount = allFindings.filter((f) => f.type === "Scrap").length;

    const passRate =
      total > 0 ? Math.round(((passed + passedWithObs) / total) * 100) : 0;
  ...
  const { periodInspections, rollOverInspections, archivedInspections } =
    useMemo(() => {
      const period = [];
      const rollOver = [];
      const archived = [];
      ...
      return {
        periodInspections: period.sort(
          (a, b) => new Date(b.inspectionDate) - new Date(a.inspectionDate),
        ),
        rollOverInspections: rollOver.sort(
          (a, b) => new Date(b.inspectionDate) - new Date(a.inspectionDate),
        ),
        archivedInspections: archived.sort(
          (a, b) => new Date(b.inspectionDate) - new Date(a.inspectionDate),
        ),
      };
  ...
  const filteredInspections = useMemo(() => {
    return displayInspections.filter((inspection) => {
      ...
      const matchesStatus =
        ...
        (activeStatusFilter === "observations" &&
          inspection.result === "Passed with Observations") ||
  ...
  const {
    visibleItems: visibleInspections,
    visibleCount: visibleInspectionsCount,
    hasMore: hasMoreInspections,
    sentinelRef: inspectionsSentinelRef,
  } = useInfiniteScroll(filteredInspections, {
    initialCount: 12,
    step: 12,
    resetKey: `${searchTerm}|${activeStatusFilter}|${selectedYear}|${selectedMonth}|${showArchive}`,
  });
  ...
  const handleEditInspection = (inspection) => {
    ...
    setNewInspection({
      ...
      inspectorId:
        staffData.find((s) => s.name === inspection.inspectorName)?.id || "",
    });
    setModalMode("edit");
    setShowModal(true);
  };
  ...
  const handleUpdateInspection = async () => {
    const result = calculateResult(
      ...
      newInspection.templateId,
    );
    const inspector = staffData.find(
      (s) => s.id === parseInt(newInspection.inspectorId),
    );
  ...
  const renderInspectionsTab = () => (
    <div className="qa-inspections">
          ...
          visibleInspections.map((inspection) => (
            <div key={inspection.id} className="qa-table-row">
              ...
              <span className="col-result">
                <span
                  className={`result-badge ${getResultClass(inspection.result)}`}
                >
                  {inspection.result}
                </span>
              </span>
      ...
      <InfiniteScrollSentinel
        ...
        visibleCount={visibleInspectionsCount}
        label="Loading more inspections..."
      />
    </div>
  );
    ...
    <div className="qa-findings">
      <div className="qa-findings-summary">
        ...
        <div className="summary-card">
          ...
          <div>
            <span className="summary-value">
              {allFindings.filter((f) => f.status === "In Progress").length}
            </span>
          ...
          allFindings.map((finding) => (
            ...
            >
              <div className="finding-header">
                ...
                <span
                  className={`finding-status ${getFindingStatusClass(finding.status)}`}
                >
                  {finding.status}
                </span>
              </div>
          ...
          <div key={template.id} className="checklist-card">
            <div className="checklist-header">
              ...
              <div className="checklist-info">
                <h4>{template.name}</h4>
                <span className="checklist-stage">
            ...
            <div className="checklist-items">
              {template.items.map((item) => (
                <div key={item.id} className="checklist-item-preview">
                  <Icon
                    name={
                      item.required ? "check_box" : "check_box_outline_blank"
                    }
                  />
            ...
            <div className="checklist-footer">
              ...
              <span>
                {template.items.filter((i) => i.required).length} required
              </span>
            </div>
          </div>
        ))}
          ...
          <div key={group.workOrderNumber} className="history-group">
            <div className="history-group-header">
              ...
              <span className="inspection-count">
                {group.inspections.length} inspection
                {group.inspections.length !== 1 ? "s" : ""}
              </span>
            </div>
              ...
              {group.inspections
                ...
                .map((inspection) => (
                  <div key={inspection.id} className="timeline-item">
                    <div
                      className={`timeline-dot ${getResultClass(inspection.result)}`}
                    ></div>
                    <div className="timeline-content">
                      <div className="timeline-header">
                        ...
                        <span
                          className={`result-badge-sm ${getResultClass(inspection.result)}`}
                        >
                          {inspection.result}
                        </span>
                      </div>
  ...
  const renderInspectionModal = () => {
    ...
    const inspection =
      modalMode === "view" ? selectedInspection : newInspection;
    const template = qaChecklistTemplates.find(
      (t) => t.id === inspection?.templateId,
    );

    return (
      <div className="modal-overlay">
        <div
          className="modal-content modal-qa"
            ...
            <div className="modal-header-text">
              <h3>
                {modalMode === "create"
                  ...
                  : modalMode === "edit"
                    ? "Edit Inspection"
                    : `Inspection - ${inspection?.workOrderNumber}`}
              </h3>
              <p>
                {modalMode === "create"
                  ? "Create a new quality inspection"
                  : modalMode === "edit"
                    ? `Editing ${inspection?.workOrderNumber}`
                ...
                {newInspection.stage && template && (
                  <div className="form-section">
                    ...
                    <div className="checklist-form">
                      {template.items.map((item) => {
                        const checklistItem = newInspection.checklist.find(
                          (c) => c.itemId === item.id,
                        ) || { checked: false, notes: "" };
                        return (
                    ...
                    <div className="findings-preview">
                      {newInspection.findings.map((finding) => (
                        <div key={finding.id} className="finding-preview-item">
                          <span
                            className={`severity-dot ${getSeverityClass(finding.severity)}`}
                          ></span>
                          <span className="finding-type-sm">
                            {finding.type}
                          </span>
                          <span className="finding-desc-sm">
                            {finding.description}
                          </span>
                        </div>
                      ))}
              ...
              <div className="inspection-view">
                <div className="view-section">
                  <div className="view-header">
                    <div className="view-result">
                      <span
                        className={`result-badge large ${getResultClass(inspection.result)}`}
                      >
                        <Icon
                          name={
                ...
                <div className="view-section">
                  ...
                  <div className="checklist-results">
                    {template?.items.map((item) => {
                      const result = inspection.checklist.find(
                        (c) => c.itemId === item.id,
                      );
                      return (
                        <div
                          key={item.id}
                          className={`checklist-result-item ${result?.checked ? "passed" : "failed"}`}
                        >
                          <Icon
                            name={result?.checked ? "check_circle" : "cancel"}
                          />
  ...
  const renderFindingModal = () => {
    ...
    return (
      <div className="modal-overlay">
        <div
          className="modal-content modal-finding"
          onClick={(e) => e.stopPropagation()}
        >
          <div className="modal-header">
        ...
        <div className="period-summary">
          {rollOverInspections.length > 0 && !showArchive && (
            <div className="rollover-badge">
              <Icon name="push_pin" />
              {rollOverInspections.length} pendiente
              {rollOverInspections.length !== 1 ? "s" : ""} de meses anteriores
            </div>
          )}
        ...
        {[
          {
            ...
            label: "Inspections",
            icon: "fact_check",
            desc: "Quality checks & reviews",
            color: "purple",
            stat: `${metrics.total} Total`,
          },
          {
            key: "findings",
            label: "Findings",
            icon: "flag",
            desc: "Issues & observations",
            color: "orange",
            stat: `${metrics.openFindings} Open`,
          },
          {
            key: "checklists",
            label: "Checklists",
            icon: "checklist",
            desc: "Templates & standards",
            color: "blue",
            stat: `${qaChecklistTemplates.length} Templates`,
          },
          {
            key: "history",
            label: "History",
            ...
            color: "green",
            stat: `${metrics.passRate}% Pass Rate`,
          },
        ].map((tab) => (
          <button
            key={tab.key}
        ...
        <div className="modal-overlay">
          ...
          >
            <div className="success-modal-content">
              <div
                className={`success-icon ${getResultClass(lastSavedInspection.result)}`}
              >
                <Icon
                  name={
                    lastSavedInspection.result === "Passed"
                      ? "check_circle"
                      : lastSavedInspection.result === "Failed"
                        ? "cancel"
                        : "info"
                  }
                />
              ...
              <p className="success-subtitle">
                {lastSavedInspection.workOrderNumber} -{" "}
                {getStageLabel(lastSavedInspection.stage)}
              </p>
              <div
                className={`success-result-badge ${getResultClass(lastSavedInspection.result)}`}
              >
              ...
              <div className="success-details">
                ...
                {lastSavedInspection.findings.length > 0 && (
                  <div className="success-detail-item">
                    <Icon name="flag" />
                    <span>
                      {lastSavedInspection.findings.length} finding(s) recorded
                    </span>
                  </div>
                )}
              </div>
