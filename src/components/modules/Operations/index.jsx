...
const OperationsModule = () => {
    ...
    const loadData = async () => {
        setIsLoading(true);
        try {
            console.log('[Operations] Loading from Supabase...');

            // Load all data in parallel
            const [opsRes, projRes, matsRes] = await Promise.all([
            ...
            if (projRes.error) console.warn('[Operations] Projects error:', projRes.error);
            if (matsRes.error) console.warn('[Operations] Materials error:', matsRes.error);

            console.log('[Operations] Loaded:', opsRes.data?.length, 'operations');
            console.log('[Operations] Loaded:', projRes.data?.length, 'projects');
            console.log('[Operations] Loaded:', matsRes.data?.length, 'materials');

            // Normalize operations from snake_case to camelCase
            const normalizedOps = (opsRes.data || []).map(op => ({
                id: op.id,
                workOrderNumber: op.work_order_number || '',
                projectId: op.project_id,
                projectName: op.name || '',
                status: op.status || 'Pending',
                priority: op.priority || 'Medium',
                dueDate: op.due_date ? op.due_date.split('T')[0] : '',
                notes: op.notes || '',
                progress: op.progress || 0,
                currentStage: op.current_stage || 'roughMill',
                assignedDivisions: op.assigned_divisions || [],
                stages: op.stages || createDefaultStages(),
                materials: op.materials || [],
                createdAt: op.created_at,
            }));

            // Normalize projects
            const normalizedProjects = (projRes.data || []).map(p => ({
                id: p.id,
                name: p.name,
                status: p.status,
            }));
            ...
            // Normalize materials
            const normalizedMaterials = (matsRes.data || []).map(m => ({
                id: m.id,
                name: m.name,
                unit: m.unit_id || 'pcs',
                stock: m.stock || 0,
            ...
            console.error('[Operations] Error loading:', error);
            setToast({ message: 'Error loading operations: ' + error.message, type: 'error' });
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleCreateOperation = async () => {
        ...
        try {
            ...
            console.log('[Operations] Created:', savedOperation.id);
    ...
    const handleDeleteSelected = async () => {
        ...
        try {
            // Delete selected operations from Supabase
            const { error } = await supabase
                .from('operations')
                .delete()
                .in('id', selectedOperations);

            if (error) throw error;
            console.log('[Operations] Deleted:', selectedOperations.length, 'operations');

            // Update local state
            setOperations(operations.filter(op => !selectedOperations.includes(op.id)));
            setSelectedOperations([]);
            setToast({ message: `${selectedOperations.length} operation(s) deleted!`, type: 'success' });
        } catch (error) {
    ...
    const handleKanbanStatusChange = async (item, newStatus) => {
        ...
        try {
            ...
            if (error) throw error;
            console.log('[Operations] Status updated:', item.id, '->', mappedStatus);
        } catch (error) {
            console.error('[Operations] Error updating status:', error);
    ...
    // Sync projects without operations
    const handleSyncProjects = async () => {
        try {
            setToast({ message: 'Syncing projects...', type: 'info' });

            // Get all project IDs that already have operations
            const existingProjectIds = operations.map(op => op.projectId).filter(Boolean);

            // Find projects that don't have operations yet
            const projectsWithoutOperations = projects.filter(p => !existingProjectIds.includes(p.id));

            if (projectsWithoutOperations.length === 0) {
                setToast({ message: 'All projects already have operations!', type: 'success' });
                return;
            }

            console.log('[Operations] Creating operations for', projectsWithoutOperations.length, 'projects');

            // Create operations for each project
            const newOperationsData = projectsWithoutOperations.map((project, index) => {
                const workOrderNumber = `OP-${new Date().getFullYear()}-${String(operations.length + index + 1).padStart(3, '0')}`;
                return {
                    work_order_number: workOrderNumber,
                    ...
                    priority: 'Medium',
                    due_date: null,
                    notes: `Auto-generated from project sync`,
                    progress: 0,
                    current_stage: 'roughMill',
                    assigned_divisions: [],
                    stages: createDefaultStages(),
            ...
            const { data: savedOperations, error } = await supabase
                .from('operations')
                .insert(newOperationsData)
                .select();

            if (error) throw error;

            console.log('[Operations] Created', savedOperations.length, 'operations');

            // Normalize and add to local state
            const normalizedNewOps = savedOperations.map(op => ({
                id: op.id,
                workOrderNumber: op.work_order_number || '',
                projectId: op.project_id,
                projectName: op.name || '',
            ...
            setOperations([...normalizedNewOps, ...operations]);
            setToast({ message: `Created ${savedOperations.length} operations for existing projects!`, type: 'success' });
        } catch (error) {
            console.error('[Operations] Error syncing projects:', error);
            setToast({ message: 'Error syncing: ' + error.message, type: 'error' });
        }
    };
        ...
        <div className="module-page operations-page">
            ...
            <div className="page-header">
                ...
                <div className="header-actions">
                    <button className="btn-secondary-action" onClick={handleSyncProjects}>
