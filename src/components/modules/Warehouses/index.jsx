...
const WarehousesModule = () => {
    ...
    // Service (resolves to the API or Supabase backend depending on config)
    const { data, loading, error, fetchAll, create, update, remove, service } = useDataService('warehouses');
    const useBackend = !!service;
    ...
    // Use backend data when available, otherwise fall back to local demo state
    const warehouses = useBackend && data?.length > 0 ? data : localWarehouses;
