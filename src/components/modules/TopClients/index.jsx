...
const TopClientsModule = () => {
    ...
    const loadClients = async () => {
        setIsLoading(true);
        try {
            // Revenue/orders aren't tracked yet, so they stay mocked; the
            // client names come from the real backend (API or Supabase).
            ...
            if (data && data.length > 0) {
                ...
                const topClientsData = data.map((client, index) => ({
                    id: client.id,
                    name: client.name || client.company_name || client.companyName || 'Client',
