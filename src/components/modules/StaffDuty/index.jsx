...
const StaffDutyModule = () => {
    ...
    const loadStaff = async () => {
        ...
        try {
            ...
            // Duty status/task/since are mocked (no attendance data yet).
            console.log('[StaffDuty] Loading staff...');
            const usersData = isApiEnabled()
                ? await usersApi.getAll()
                : await profilesService.getAll();
            console.log('[StaffDuty] Response:', usersData);
            if (usersData?.length > 0) {
                // Transform users to staff duty format
                const staffDuty = usersData.map((user, index) => {
                    const name = user.name || user.display_name || user.username || 'Unknown';
