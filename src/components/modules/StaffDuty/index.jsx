...
const StaffDutyModule = () => {
    ...
    const loadStaff = async () => {
        setIsLoading(true);
        try {
            console.log('[StaffDuty] Loading users from Supabase...');
            ...
            console.log('[StaffDuty] Supabase response:', usersData);

            if (usersData?.length > 0) {
                // Transform users to staff duty format
                const staffDuty = usersData.map((user, index) => {
                    const name = user.display_name || user.username || user.email || 'Unknown';
