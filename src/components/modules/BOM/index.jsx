 ...
 * BOM Status options
 */
const bomStatusOptions = ['Draft', 'Active', 'Obsolete'];
...
const BOMModule = () => {
    ...
    /**
     * Load BOMs, products and materials from Supabase
     */
    const loadData = async () => {
        setIsLoading(true);
        try {
            console.log('[BOM] Loading data from Supabase...');

            // Load all data in parallel
            const [bomsRes, productsRes, materialsRes] = await Promise.all([
                supabase.from('bom').select('*').order('created_at', { ascending: false }),
            ...
            console.log('[BOM] Loaded:', bomsRes.data?.length, 'BOMs');
            console.log('[BOM] Loaded:', productsRes.data?.length, 'products');
            console.log('[BOM] Loaded:', materialsRes.data?.length, 'materials');

            // Set products and materials first
            setProducts(productsRes.data || []);
            setMaterials(materialsRes.data || []);

            // Normalize and set BOMs with their components
            if (bomsRes.data?.length > 0) {
                const bomsWithComponents = await Promise.all(
                    bomsRes.data.map(async (bom) => {
                        // Load components for each BOM
                        ...
                        return normalizeBom(bom, componentsData || []);
                    })
                );
                setBoms(bomsWithComponents);
            } else {
                setBoms([]);
            }
        } catch (error) {
            console.error('[BOM] Error loading data:', error);
            setToast({ message: 'Error loading BOMs: ' + error.message, type: 'error' });
            setBoms([]);
        } finally {
            setIsLoading(false);
        }
    ...
    /**
     * Normalize BOM data from Supabase to frontend format
     */
    const normalizeBom = (bom, components = []) => {
        // Map components from snake_case to camelCase
        const normalizedComponents = components.map(comp => ({
            id: comp.id,
            type: comp.component_type || 'material',
            itemId: comp.material_id,
            itemName: '', // Will be filled later
            itemCode: '', // Will be filled later
            quantity: comp.quantity || 1,
            unit: '', // Will be filled later
            unitCost: parseFloat(comp.unit_cost) || 0,
            totalCost: parseFloat(comp.total_cost) || 0,
            stockAvailable: 0,
            stockStatus: 'sufficient',
            notes: comp.notes || '',
            subBomId: comp.sub_bom_id || null
        }));
        ...
        return {
            ...
            code: bom.code || '',
            name: bom.name || '',
            productId: bom.product_id || null,
            productName: '', // Will be filled from products
            description: bom.description || '',
            version: bom.version || '1.0',
            status: bom.status || 'Draft',
            components: normalizedComponents,
            totalMaterialCost: parseFloat(bom.total_material_cost) || 0,
            laborCost: parseFloat(bom.labor_cost) || 0,
            overheadCost: parseFloat(bom.overhead_cost) || 0,
            totalCost: parseFloat(bom.total_cost) || 0,
            margin: parseFloat(bom.margin) || 30,
            suggestedPrice: parseFloat(bom.suggested_price) || 0,
            isActive: bom.is_active !== false,
    ...
    const confirmDelete = async () => {
        if (bomToDelete) {
            try {
                console.log('[BOM] Deleting BOM:', bomToDelete.id);

                // First delete components
                const { error: compError } = await supabase
                    .from('bom_components')
                    .delete()
                    .eq('bom_id', bomToDelete.id);

                if (compError) {
                    console.warn('[BOM] Error deleting components:', compError);
                }
                ...
                setToast({ message: 'BOM deleted successfully!', type: 'success' });
            } catch (error) {
                console.error('[BOM] Error deleting:', error);
                setToast({ message: 'Error deleting BOM: ' + error.message, type: 'error' });
            }
        }
    ...
    const handleSave = async () => {
        try {
            console.log('[BOM] Saving BOM to Supabase...');
            ...
            const bomToSave = {
                ...
                product_id: currentBOM.productId || null,
            ...
            if (modalMode === 'add') {
                console.log('[BOM] Creating new BOM...');
                ...
                if (error) throw error;
                savedBomId = newBOM.id;
                console.log('[BOM] Created:', savedBomId);
            } else if (modalMode === 'edit') {
                console.log('[BOM] Updating BOM:', currentBOM.id);
                const { error } = await supabase
                    .from('bom')
                    .update({ ...bomToSave, updated_at: new Date().toISOString() })
                    .eq('id', currentBOM.id);

                if (error) throw error;
                savedBomId = currentBOM.id;

                // Delete existing components to replace them
                await supabase
                    .from('bom_components')
                    .delete()
                    .eq('bom_id', currentBOM.id);
            }

            // Save components
            if (currentBOM.components && currentBOM.components.length > 0 && savedBomId) {
                const componentsToInsert = currentBOM.components
                    .filter(comp => comp.itemId || comp.subBomId)
                    .map((comp, index) => ({
                        bom_id: savedBomId,
                        material_id: comp.type === 'material' ? comp.itemId : null,
                        sub_bom_id: comp.type === 'subassembly' ? comp.subBomId : null,
                        component_type: comp.type || 'material',
                        quantity: parseFloat(comp.quantity) || 1,
                        unit_cost: parseFloat(comp.unitCost) || 0,
                        total_cost: parseFloat(comp.totalCost) || 0,
                        notes: comp.notes || '',
                        sort_order: index
                    }));

                if (componentsToInsert.length > 0) {
                    console.log('[BOM] Saving', componentsToInsert.length, 'components');
                    const { error: compError } = await supabase
                        .from('bom_components')
                        .insert(componentsToInsert);

                    if (compError) {
                        console.error('[BOM] Error saving components:', compError);
                    }
                }
            }

            console.log('[BOM] Save successful!');
            setToast({ message: modalMode === 'add' ? 'BOM created successfully!' : 'BOM updated successfully!', type: 'success' });
            setShowModal(false);
            ...
            setToast({ message: 'Error saving BOM: ' + error.message, type: 'error' });
        }
    };
    ...
    const handleComponentChange = (field, value) => {
        setCurrentComponent(prev => {
            ...
            if (field === 'subBomId' && prev.type === 'subassembly') {
                const subBom = boms.find(b => b.id === value);
                if (subBom) {
