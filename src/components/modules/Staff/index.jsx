...
const StaffModule = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // REST API uses /users; Supabase stores staff in the profiles table
      const usersData = isApiEnabled()
  ...
  const handleUpdateUser = async () => {
    ...
    try {
      ...
      if (isApiEnabled()) {
        ...
        // API disabled → persist to the Supabase `profiles` table. The
        // update used to only run through the (disabled) phantom API, so
        // editing a user never saved anything. Write both name and
        // display_name because the list reads display_name first.
        // Password is an Auth credential and can't be changed with the
        // anon key (needs the server-side admin key), so it's skipped.
        ...
        if (newUser.password?.trim()) {
          alert(
            "Datos guardados. La contraseña no se puede cambiar desde aquí (requiere permisos de servidor).",
          );
        }
      }
      setNewUser({ username: "", email: "", password: "", role: "" });
