...
import { profilesService, supabase } from "../../../lib/supabase";

// No local data - all data comes from Supabase (profiles table).
// Privileged user ops (create/update/delete/reset password) go through the
// `admin-users` Edge Function, which holds the service_role server-side.
// supabase-js attaches the current session JWT automatically.
...
// Invoke the admin-users Edge Function and surface its JSON error message.
const invokeAdminUsers = async (body) => {
  const { data, error } = await supabase.functions.invoke("admin-users", {
    body,
  });
  if (error) {
    // supabase-js wraps non-2xx responses; dig out the function's {error}.
...
const StaffModule = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Supabase stores staff in the profiles table.
      const usersData = await profilesService.getAll();
      console.log("[Staff] Response:", usersData);
      if (usersData?.length > 0) {
        // Normalize user data
        const normalizedUsers = usersData.map((u) => ({
          id: u.id || u.idUser,
          username: u.username || u.display_name || u.name || "",
          email: u.email || "",
          role: u.role || "USER",
  ...
  const handleCreateUser = async () => {
    if (
      ...
      return;
    try {
      // The front's "username" maps to profiles.name / display_name.
      console.log("[Staff] Creating user:", newUser.email);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await invokeAdminUsers({ action: "delete", id: userToDelete.id });
  ...
  const handleUpdateUser = async () => {
    ...
    try {
      ...
      // Add password only if the admin typed a new one.
      if (newUser.password?.trim()) {
        payload.password = newUser.password;
      }

      console.log("[Staff] Updating user:", editingUser.id);
  ...
  const adminCount = users.filter(
    (u) => u.role === "ADMIN" || u.role === "ADMIN_DEV",
  ).length;
        ...
        <div className="staff-list-container">
          <div className="staff-cards-grid">
            {sortedUsers.map((user) => (
              <div
                key={user.id}
                className={`staff-card ${selectedUsers.includes(user.id) ? "selected" : ""}`}
              >
                ...
                <div className="staff-card-body">
                  ...
                  <span
                    className={`staff-card-role role-${user.role.toLowerCase().replace("_", "-")}`}
                  >
                    {user.role}
                  </span>
