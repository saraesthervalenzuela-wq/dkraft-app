...
const ProductsModule = () => {
  ...
  useEffect(() => {
    ...
    const pendingProducts = products.filter(
      (p) => !p.qbListId && p.qbSyncStatus !== "error",
    );
    setPendingSyncCount(pendingProducts.length);

    if (pendingProducts.length > 0) {
      console.log(
        `[Products] ${pendingProducts.length} products pending QB sync, starting polling...`,
      );
        ...
        try {
          ...
          if (updatedProducts?.length > 0) {
            ...
            if (newlySynced.length > 0) {
              console.log(
                `[Products] ${newlySynced.length} products synced with QB:`,
                newlySynced.map((p) => p.name),
              );
            }
            ...
            const stillPending = normalizedProducts.filter(
              (p) => !p.qbListId && p.qbSyncStatus !== "error",
            );
    ...
    try {
      ...
      if (isApiEnabled()) {
        try {
          ...
          console.warn(
            "[Products] API error, falling back to localStorage:",
            apiError.message,
          );
        }
      } else {
        // Use Supabase when the REST API is disabled
        try {
          [productsData, categoriesData] = await Promise.all([
          ...
          console.warn(
            "[Products] Supabase error, falling back to localStorage:",
            sbError.message,
          );
        }
      }
  ...
  const getQBSyncStatusFromData = (product) => {
    if (product.qbListId || product.qb_list_id) return "synced";
    if (product.qbSyncStatus === "error" || product.sync_status === "error")
      return "error";
    return "pending";
  };
  ...
  const normalizeProduct = (p) => {
    ...
    // API uses camelCase; Supabase uses snake_case columns.
    const costPrice = parseFloat(p.costPrice ?? p.cost_price) || 0;
    ...
    return {
      ...
      categoryId:
        p.categoryId || p.category_id || findCategoryId(p.category) || "",
      status: normalizeStatus(p.status),
      costPrice: costPrice,
      price: price,
      qbSyncStatus: qbSyncStatus,
      qbListId: p.qbListId || p.qb_list_id || null,
      qbEditSequence: p.qbEditSequence || p.qb_edit_sequence || null,
      account: p.account || p.accountId || findAccountId(p.account) || "",
      currency: currencyValue,
      deleted: p.deleted || p.is_deleted || false,
    };
  };
  ...
  const handleSave = async () => {
    try {
      ...
      if (modalMode === "add") {
        ...
        // Update in Supabase
        savedProduct = await productsService.update(
          currentProduct.id,
          productData,
        );
  ...
  return (
    <div className="module-page products-page">
        ...
        <div className="header-actions">
          {pendingSyncCount > 0 && (
            <div
              ...
              title={`${pendingSyncCount} products pending QB sync`}
            >
      ...
      {isLoading ? (
        ...
        /* Cards View — grid grows with the page, so the window scroll
           drives the sentinel placed below it */
        <>
                ...
                <div className="material-card-footer">
                  <div className="material-stock">
                    ...
                    <span className="stock-value">
                      {getAccountName(product.account)}
                    </span>
                  </div>
        ...
        <div className="products-table-container">
          <div className="products-table">
            <div className="products-table-header">
              ...
              <span className="col-status-qb" title="QuickBooks Status">
                QB
              </span>
              <span
          ...
          {/* Sentinel lives INSIDE the scroll container so the inner table
              scroll (max-height: 100vh-280px), not the window, drives loading */}
          <InfiniteScrollSentinel
            sentinelRef={productsSentinelRef}
            hasMore={hasMoreProducts}
            total={sortedProducts.length}
            ...
            <div className="form-group">
              ...
              <div className="qb-status-display">
                <Icon
                  ...
                  style={{
                    color: getQBStatusIcon(currentProduct.qbSyncStatus).color,
                  }}
                />
                <span>
                  {getQBStatusIcon(currentProduct.qbSyncStatus).label}
                </span>
              </div>
            </div>
          )}
